...
export const loadTokens = async (provider, chainId, dispatch) => {
	const henry = new ethers.Contract(config[chainId].henry.address, TOKEN_ABI, provider)
	const usd = new ethers.Contract(config[chainId].usd.address, TOKEN_ABI, provider)

	dispatch(setContracts([henry, usd]))
	dispatch(setSymbols([await henry.symbol(), await usd.symbol()]))
}
