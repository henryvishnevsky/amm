...
async function main() {
  ...
  // Fetch Henry Token
  const henry = await ethers.getContractAt('Token', config[chainId].henry.address)
  console.log(`Henry Token fetched: ${henry.address}\n`)
  ...
  // Send henry tokens to investor 1
  transaction = await henry.connect(deployer).transfer(investor1.address, tokens(10))
  ...
  // Send henry tokens to investor 3
  transaction = await henry.connect(deployer).transfer(investor3.address, tokens(10))
  await transaction.wait()
  ...
  transaction = await henry.connect(deployer).approve(amm.address, amount)
  await transaction.wait()
  ...
  /////////////////////////////////////////////////////////////
  // Investor 1 Swaps: HENRY --> USD
  //

  console.log(`Investor 1 Swaps...\n`)

  // Investor approves all tokens
  transaction = await henry.connect(investor1).approve(amm.address, tokens(10))
  await transaction.wait()
  ...
  /////////////////////////////////////////////////////////////
  // Investor 3 Swaps: HENRY --> USD
  //

  console.log(`Investor 3 Swaps...\n`)

  // Investor approves all tokens
  transaction = await henry.connect(investor3).approve(amm.address, tokens(10))
  await transaction.wait()
